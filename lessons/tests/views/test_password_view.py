...
class PasswordViewTest(TestCase):
    ...
    def test_succesful_password_change_for_students(self):
        self.client.login(username=self.user.email, password='Password123')
    ...
    def test_password_change_unsuccesful_without_correct_old_password_for_students(self):
        self.client.login(username=self.user.email, password='Password123')
    ...
    def test_password_change_unsuccesful_without_password_confirmation_for_students(self):
        self.client.login(username=self.user.email, password='Password123')
    ...
    def test_succesful_password_change_for_admins(self):
    ...
    def test_password_change_unsuccesful_without_correct_old_password_for_admins(self):
        self.client.login(username=self.admin.email, password='Password123')
        self.form_input['password'] = 'WrongPassword123'
        response = self.client.post(self.url, self.form_input, follow=True)
        self.assertEqual(response.status_code, 200)
    ...
    def test_password_change_unsuccesful_without_password_confirmation_for_admins(self):
        self.client.login(username=self.admin.email, password='Password123')
        self.form_input['password_confirmation'] = 'WrongPassword123'
        response = self.client.post(self.url, self.form_input, follow=True)
        self.assertEqual(response.status_code, 200)
